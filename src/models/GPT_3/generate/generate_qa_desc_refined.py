...
def generate_score_org(model, tokenizer, args, device, input_str, eval_str):
    #penalty on same word
            ...
            for w in range(size):
                ...
                for jj in gen:
                    if ('<|end' in jj[0]) or (i>30):
                        ...
                        sc=sc.item()
                        sc=sc-0.01*i  #the longer the better.
                        if i>45:
                            sc+=35
                        if sc>best_score:
                            best_score=sc
...
def generation_interface(model,tokenizer,args,device,question,desc):
    output_string,output_scores=generate_string(model, tokenizer, args, device,question,desc)
    ranklist=np.argsort(output_scores)

    return output_string[ranklist[0]]

def qa_desc(model,tokenizer,args, content):
    content_l = content.split("&&")
    question = content_l[0]
    desc = content_l[1]
