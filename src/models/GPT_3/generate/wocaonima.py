...
def generate_sentence(model, tokenizer, args, device, input,max_tries=1000):
    ...
    with torch.no_grad():
        ...
        if context_length > max_seq_len:
            ...
            context_tokens = context_tokens[0:max_seq_len-2] + context_tokens[-2:]
        context_tokens_tensor = torch.cuda.LongTensor(context_tokens)
        ...
        while ((tries < max_tries or num_outs == 0) and tries < 250000):
            ...
            index = len(tokens[0])
            print(tokens.shape)
            print((tokens.new_ones((1, 1)) * (index - 1)))
            print(tokens.new_ones(1, 1, 1, args.mem_length + 1).shape)
...
def dl_interface(model, tokenizer, args, device, content, max_tries):
    ...
    # 2:出句为上联  1：出句为下联
    output_string= generate_sentence(model, tokenizer, args, device, content, max_tries)
...
def generate_duilian(content,model, tokenizer, args,max_tries):
    ...
    # wq = checkpz(pp[-1][0], content[-1])
    # 2:出句为上联  1：出句为下联
    best_duiju = dl_interface(model, tokenizer, args, torch.cuda.current_device(), content,max_tries)
...
if __name__ == "__main__":
    ...
    model, tokenizer, args = prepare_model()
    user_texts = os.listdir("/home/huangs/depressiondetection/data/weibo2012/processed/texts/")
    for each_user in user_texts:
        with open(os.path.join("/home/huangs/depressiondetection/data/weibo2012/processed/texts/", each_user), "r") as f:
            content = "".join(f.readlines())
            logits = generate_duilian(content, model, tokenizer, args,1000)
            result = np.zeros([max_seq_len, 2560])
            result[:logits.shape[1],:] = logits.cpu().numpy()[0,:,:]
            #print(result.shape)
            np.save(os.path.join("/home/huangs/depressiondetection/data/weibo2012/processed/WudaoEmbeddings/", each_user[:-3] + "npy"), result)