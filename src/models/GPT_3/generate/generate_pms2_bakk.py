...
def generate_strs(tups):
    ...
    for tup in tups:
        ...
        output_string,output_scores=generate_string(model,tokenizer, args, torch.cuda.current_device(),tup[0],tup[1])
        list_poems=0

        ranklist=np.argsort(output_scores)
        best_score=output_scores[ranklist[0]]
        text_dir="poems_save_1/"
        already=[]
...
def generate():
    ...
    title_list=["咏手机","论因果分析","咏深度学习","咏神经网络"]

    author_list=["唐 王昌龄","唐 李白","唐 杜甫","晋 陶渊明","宋 苏轼","清 曹雪芹","唐 温庭筠","唐 贺知章","唐 李商隐","唐 孟浩然","宋 欧阳修","宋 曾巩","宋 王安石","楚 屈原","楚 宋玉","汉 孔融","汉 杨修","魏 曹操","魏 曹丕","魏 曹植","魏 王粲","魏 陈琳","吴 诸葛恪","晋 嵇康","晋 潘安","晋 谢灵运","晋 谢安","梁 萧衍","唐 陈子昂","唐 王维","唐 骆宾王","唐 杜牧","唐 白居易","唐 刘禹锡","唐 柳宗元","南唐 李煜","宋 苏轼","宋 李清照","宋 文天祥","宋 苏辙","宋 辛弃疾","宋 岳飞","宋 秦桧","宋 蔡京","明 于谦","明 严世蕃","清 袁枚","清 纪晓岚","清 康有为","清 纳兰性德","清 爱新觉罗弘历","民国 张宗昌"]

    for j in author_list[1:4]:
        for i in title_list:
            fi.append([i,j])
...
def write_common(content,model, tokenizer, args):
    ...
    output_string,output_string_score = generate_string(model, tokenizer, args, torch.cuda.current_device(),title,author)
    minpos=np.argmin(output_string_score)
    print("output over : "+title + " " + author)
    print(output_string[minpos])
    return output_string[minpos]
