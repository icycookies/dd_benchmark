...
def set_args():
    ...
    args.load = 'data/checkpoints_kws/txl-2.8b11-20-15-10'
    # shixw
...
def generate_kws_abs_title(model, tokenizer, args,content):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #print(count)
        b = time.time()
        content = "摘要:金钗石斛是历代《中国药典》收载的中药石斛基源植物和国家二级珍稀濒危兰科药用植物、富含石斛碱，经济价值和市场需求巨大。由于其繁殖力低、组培苗移栽成活率低、药用成分含量低、人工培养成本高，自然条件下需与真菌共生才能完成其生活史。因此，寻求促其生长和品质提高的菌根真菌是该领域研究和开发的首选。本课题组从兰科植物中分到促进金钗石斛生长和生物碱含量提高的真菌MF23(Mycena sp.)，究其分子作用机制不详并未见报道；导致使用该菌在诱导时间和采收石斛部位等方面把握不足，严重影响该菌的推广应用。鉴如此，本项目拟在前期研究基础上，研究该菌诱导金钗石斛生物碱合成途径中三种关键酶活性变化；并在基因转录水平和蛋白质表达水平上，研究这几个酶基因受真菌诱导的表达特征和表达功能；以期探究真菌诱导金钗石斛生物碱合成的分子作用与表达机制，为利用内生真菌促进金钗石斛生产提供科学依据，为菌根互作研究提供分子生物学资料。 关键词:"
        output = generate_common(content,model,tokenizer,args)
        #print("output: " + output)
        #print("consuming: " + str(time.time()-b))
        #print("----------------------------------------------------------")
