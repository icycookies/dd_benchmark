...
def prepare_model(iteration=160000):
    ...
    #generate samples
    return model_1,model_2,tokenizer,args
...
def generate_common(content,model, tokenizer, args):
...
if __name__ == "__main__":
    content_1 = "咏奥巴马 作者：李白"
    content_2 = "大鹅赞歌 作者：杜甫" 
      
    model_1,model_2,tokenizer, args = prepare_model()
    t1 = threading.Thread(target=generate_common,args=(content_1,model_1,tokenizer,args))
    t2 = threading.Thread(target=generate_common,args=(content_2,model_2,tokenizer,args))
    t1.start()
    t2.start()
