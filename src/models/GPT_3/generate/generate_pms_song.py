...
def checkrhyself(sentence,pro,yayun=None):
    ...
    for i in range(len(st)):
        ...
        #print(i,st[i],pz)
        if not(l1[i][0][-1] in ['1','2','3','4']) and (pro[i]=='中'):
            return 1
...
def generate_sentence(model,tokenizer,args,device,current_tokens,mems,endnote=[",","，","?","？"],num_candidates=10,min_length=5,max_length=7,heading=None,pron=None,yayun=None):
    ...
    with torch.no_grad():
        ...
        if add_sec:
            org=original_context.split('正文:')
            in_st=org[0]+'正文:'+getlastsentence(original_context,length=20)
...
def generate_string(model, tokenizer, args, cploader, device,title,author,cipai,desc=None,length=None,headings=None):
    input_str=title+' 词牌:'+cipai+" 作者:"+author+"  题名:"+title+" 描述:"+desc+" 正文: "
    ...
    with torch.no_grad():
        ...
        for w in range(len(beam_sentences)):
            if '<|end' in beam_sentences[w][0]:
                continue
            if hd==0:
                input='”'+beam_sentences[w][0]+'”此句出自宋词《'+cipai+'·'
            else:
                input='”'+headings+pron_chr[0]+beam_sentences[w][0]+'”此句出自宋词《'+cipai+'·'
            output_str=title+'》'
